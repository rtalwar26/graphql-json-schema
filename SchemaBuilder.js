    ...
    switch (type) {
      ...
      case "array":
        return new graphql.GraphQLList(
          exports.getGQLType(configPath, name, field.items, isInput)
        );
      default:
...
exports.getGqlFields = (parentname, configPath, schema, isInput = false) => {
  ...
  let isImported =
    typeof properties === "string" && properties.startsWith("require:");
  ...
  for (let key in properties_data) {
    ...
    if (isKeyImported) {
      ...
      importedFields = Object.assign(
        Object.assign({}, importedFields),
        propertyData
      );
    } else {
      fields[key] = {
        type: exports.getGQLType(
          fieldConfigPath,
          `${parentname}_${key}`,
          properties_data[key],
          isInput
